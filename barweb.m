...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
 ...
 % this if conditons is added for the the element of error with all
 % zeros.(Zonglei Zhen,20080415)
 % if any element is nonzero, display the error bar; when all element is
 % zeros,do not display errorbar 
 if  any(any(errors))
	for i = 1:numbars
